    ...
    class AnnoSetupFilter : IStartupFilter
    {
        ...
        private async Task ApiInvoke(HttpContext context, Func<Dictionary<string, string>, string> invoke)
        {
            ...
            try
            {
                ...
                {
                    input.Add("X-Original-For", headers["X-Original-For"].ToArray()[0].Replace("::ffff:", ""));
                }
                else if (headers != null && headers.ContainsKey("X-Forwarded-For") && !headers["X-Forwarded-For"].ToArray()[0].StartsWith("127.0.0.1"))
                {
                    input.Add("X-Original-For", headers["X-Forwarded-For"].ToArray()[0].Replace("::ffff:", ""));
                }
                else
                {
